...
const isHttpUrl = (value: string): boolean => /^https?:\/\//i.test(normalizeText(value));
...
const formatSourceName = (value: unknown): string => {
  const text = normalizeText(value);
  if (!text || PLACEHOLDER_SOURCE.test(text)) return '';
...
export const resolveJobSourceAttribution = (job: {
...
}): JobSourceAttribution | null => {
  const sourceName = formatSourceName(job?.sourceName ?? job?.source);
  ...
  if (href) {
    ...
    if (host === 'jobsinvizag.in') return null;
  }
