...
type Props = NativeStackScreenProps<RootStackParamList, 'EmployerResetPassword'>;

export default function EmployerResetPasswordScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (ignore) return;
      if (event === 'PASSWORD_RECOVERY' || nextSession) setRecoveryReady(true);
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        // Password is already updated even if the local sign-out fails.
      }
        ...
        {!canReset ? (
          <>
            ...
            <Pressable
              ...
              onPress={() => navigation.replace('EmployerForgotPassword')}
            >
          ...
          <>
            <Text style={styles.subtitle}>Enter a new password for your employer account.</Text>
            <FormField
              label="New password"
              secureTextEntry
              value={password}
